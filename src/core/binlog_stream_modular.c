...
static uint32_t g_enum_cache_size = 0;
...
static void free_enum_cache(void) {
    ...
    for (uint32_t i = 0; i < g_enum_cache_size; i++) {
...
static int load_config(const char *filename, config_t *cfg) {
                            ...
                            json_object_object_foreach(tbl_wrapper, tbl_name, tbl_obj) {
                                ...
                                if(columns && json_object_is_type(columns, json_type_array)) {
                                    ...
                                    for(int k = 0; k < col_count; k++) {
                                        ...
                                        strncpy(tbl_cfg->columns[k].name, col_name, sizeof(tbl_cfg->columns[k].name) - 1);
    ...
    log_info("Binlog: %s @ %lld", cfg->binlog_file[0] ? cfg->binlog_file : "current", (long long)cfg->binlog_position);
    log_info("Save position every: %d events", (int)cfg->save_position_event_count);
...
/* Return 1 if the server is MariaDB, 0 otherwise */
static int server_is_mariadb(MYSQL *mysql)
{
    if (!mysql) return 0;
    if (mysql_query(mysql, "SELECT VERSION()") != 0) return 0;
    ...
    int is_maria = 0;
    if (row && row[0] && strstr(row[0], "MariaDB") != NULL) is_maria = 1;
...
static void announce_checksum(MYSQL *mysql)
{
    ...
    (void)mysql_query(mysql, "SET @source_binlog_checksum = @@GLOBAL.binlog_checksum");
    (void)mysql_query(mysql, "SET @master_binlog_checksum = @@GLOBAL.binlog_checksum");
...
static int get_master_position(MYSQL *mysql, char *file_out, size_t file_size, uint64_t *pos_out)
{
                                       ...
                                       enum_cache_t *cache)
{
    ...
    cache->loaded = 1;
    log_trace("Loaded %d ENUM values for %s.%s.%s", cache->count, db, tbl, col_name);
...
static void fetch_column_names(const char *db, const char *tbl) {
    ...
    if(g_map.column_names) {
        for(uint32_t i = 0; i < g_map.ncols; i++) free(g_map.column_names[i]);
    ...
    if(g_map.column_names) {
        ...
        for(int i = 0; i < num_fields; i++) g_map.column_names[i] = strdup(fields[i].name);
...
static void parse_table_map(const unsigned char *p, uint32_t len)
{
    ...
    if ((size_t)(p - (const unsigned char *)0) == 0) { /* no-op to silence some analyzers */ }
    ...
    /*
     * IMPORTANT FIX:
     * Determine "table changed" BEFORE overwriting g_map.db/g_map.tbl
     * and aggressively invalidate cached column names + enum cache when it changes.
     */
    int table_changed =
        (g_map.column_names_fetched == 0) ||
        (g_map.table_id != tid) ||
        (strcmp(g_map.db, new_db) != 0) ||
        (strcmp(g_map.tbl, new_tbl) != 0);

    if (table_changed) {
        /* clear cached column names from previous table */
        if (g_map.column_names) {
        ...
        /* clear enum cache (per-column) from previous table */
        free_enum_cache();
        g_enum_cache = NULL;
        g_enum_cache_size = 0;
    }

    /* now it is safe to update the global table map identity */
    g_map.table_id = tid;
    ...
    /* ensure we have a txn id */
    if (!in_transaction) {
        generate_txn_id(current_txn_id);
        in_transaction = 1;
    }

    /* reset per-table map arrays */
    free(g_map.types);
    free(g_map.metadata);
    ...
    /* allocate enum cache for this table if needed */
    if (g_enum_cache == NULL && g_map.ncols > 0) {
        g_enum_cache = (enum_cache_t *)calloc(g_map.ncols, sizeof(enum_cache_t));
        g_enum_cache_size = g_map.ncols;
    }
    ...
    if (!g_map.types || !g_map.metadata || !g_map.real_types) {
        log_error("TABLE_MAP: OOM allocating type/meta arrays (ncols=%u)", g_map.ncols);
        return;
    }
    ...
    /* metadata length (lenenc-int style used in binlog rows events) */
    uint64_t meta_len = 0;
    if (*p < 251) {
        meta_len = *p++;
    } else if (*p == 252) {
        ...
        switch (t) {
            ...
            case MT_STRING:
                if (p + 1 < meta_start + meta_len) {
                    ...
                    /* decode real type for STRING (ENUM/SET is encoded here) */
    ...
    /* fetch/refresh column names for this table */
    fetch_column_names(g_map.db, g_map.tbl);

    /* map configured columns (or wildcard) to indices */
    table_config_t *tbl_cfg = find_table_config(g_map.db, g_map.tbl);
    if (tbl_cfg && g_map.column_names) {
        if (tbl_cfg->capture_all_columns) {
            /* rebuild columns array to match actual ncols */
            free(tbl_cfg->columns);
            tbl_cfg->column_count = (int)g_map.ncols;
            tbl_cfg->columns = (column_info_t *)calloc(g_map.ncols, sizeof(column_info_t));
            if (tbl_cfg->columns) {
                for (uint32_t i = 0; i < g_map.ncols; i++) {
                    if (g_map.column_names[i]) {
                        strncpy(tbl_cfg->columns[i].name, g_map.column_names[i],
                                sizeof(tbl_cfg->columns[i].name) - 1);
                        tbl_cfg->columns[i].index = (int)i;
                        tbl_cfg->columns[i].position = (int)i;
                    }
                }
            }
        } else {
            for (int i = 0; i < tbl_cfg->column_count; i++) {
                ...
                for (uint32_t j = 0; j < g_map.ncols; j++) {
                    if (g_map.column_names[j] &&
                        ...
                        tbl_cfg->columns[i].index = (int)j;
                        log_trace("Mapped column %s to index %u", tbl_cfg->columns[i].name, j);
                        break;
                    }
                }
    ...
    log_debug("[txn:%s] TABLE_MAP tid=%llu db='%s' table='%s' ncols=%u (table_changed=%d)",
              current_txn_id, (unsigned long long)tid, g_map.db, g_map.tbl, g_map.ncols, table_changed);
}

/* Number of bytes used to store N decimal digits (0..9) in NEWDECIMAL */
static const uint8_t dig2bytes[10] = { 0,1,1,2,2,3,3,4,4,4 };

static uint32_t read_be_uint(const unsigned char *p, int bytes)
{
    uint32_t v = 0;
...
/*
 * Decode MySQL binary NEWDECIMAL (a.k.a MYSQL_TYPE_NEWDECIMAL) as used in row events.
 * precision = meta & 0xFF
 * scale     = (meta >> 8) & 0xFF
 *
 * Returns:
 *   bytes_consumed (>0) on success
 *   0 on failure
 *
 * Output is a *decimal text* (no quotes). You can wrap it in JSON quotes if desired.
 */
static int newdecimal_bin_to_str(const unsigned char *bin,
                                int precision,
                                int scale,
                                char *out,
                                size_t outsz)
{
    if (!bin || !out || outsz == 0) return 0;
    if (precision < 1 || precision > 65) return 0;
    if (scale < 0 || scale > precision) return 0;
    ...
    int intg0_bytes = dig2bytes[intg0];
    int frac0_bytes = dig2bytes[frac0];

    int total_bytes = intgX * 4 + intg0_bytes + fracX * 4 + frac0_bytes;
    if (total_bytes <= 0 || total_bytes > 64) return 0;

    /* Copy so we can mutate (sign handling / inversion) */
    unsigned char tmp[64];
    memcpy(tmp, bin, (size_t)total_bytes);

    /*
     * MySQL stores sign in the highest bit of the first byte:
     *  - positive: sign-bit set after XOR 0x80
     *  - negative: sign-bit not set, and value bytes are inverted after sign toggle
     */
    int positive = (tmp[0] & 0x80) ? 1 : 0;
    tmp[0] ^= 0x80;
    if (!positive) {
        for (int i = 0; i < total_bytes; i++) tmp[i] ^= 0xFF;
    }

    size_t off = 0;

    if (!positive) {
        if (off + 1 >= outsz) return 0;
        out[off++] = '-';
    }

    /* Parse integer groups */
    const unsigned char *p = tmp;
    int wrote_int = 0;

    if (intg0 > 0) {
        uint32_t v = read_be_uint(p, intg0_bytes);
        p += intg0_bytes;

        /* First (partial) group prints without left padding */
        char buf[16];
        int n = snprintf(buf, sizeof(buf), "%u", v);
        if (n < 0) return 0;
        if (off + (size_t)n >= outsz) return 0;
        memcpy(out + off, buf, (size_t)n);
        off += (size_t)n;
        wrote_int = 1;
    }

    for (int i = 0; i < intgX; i++) {
        uint32_t v = read_be_uint(p, 4);
        p += 4;

        if (!wrote_int) {
            /* First full group prints without padding */
            char buf[16];
            int n = snprintf(buf, sizeof(buf), "%u", v);
            if (n < 0) return 0;
            if (off + (size_t)n >= outsz) return 0;
            ...
            /* Subsequent full groups pad to 9 digits */
            char buf[16];
            int n = snprintf(buf, sizeof(buf), "%09u", v);
            if (n < 0) return 0;
            if (off + (size_t)n >= outsz) return 0;
            memcpy(out + off, buf, (size_t)n);
            off += (size_t)n;
    ...
    if (!wrote_int) {
        /* If integer part is zero and no integer group printed */
        if (off + 1 >= outsz) return 0;
        out[off++] = '0';
    }
    ...
    if (scale > 0) {
        ...
        if (frac0 > 0) {
            uint32_t v = read_be_uint(p, frac0_bytes);
            p += frac0_bytes;

            /* Must pad fractional remainder to exactly frac0 digits */
            char fmt[16];
            snprintf(fmt, sizeof(fmt), "%%0%du", frac0);
            char buf[16];
...
// ============================================================================
// COLUMN VALUE PARSER (includes REAL NEWDECIMAL conversion)
// ============================================================================
    ...
    const char *col_name)
{
    ...
    *offset += snprintf(json_buf + *offset, buf_size - *offset, "\"%s\":", col_name);
    ...
    switch (real_type) {
        case MT_TINY:
            *offset += snprintf(json_buf + *offset, buf_size - *offset, "%d", (int8_t)*p);
            return p + 1;

        case MT_SHORT:
        case MT_YEAR:
            *offset += snprintf(json_buf + *offset, buf_size - *offset, "%d", (int16_t)le16(p));
            return p + 2;

        case MT_INT24: {
            int32_t v = (int32_t)le24(p);
        ...
        case MT_LONG:
            *offset += snprintf(json_buf + *offset, buf_size - *offset, "%u", le32(p));
            return p + 4;

        case MT_LONGLONG:
            *offset += snprintf(json_buf + *offset, buf_size - *offset, "%llu",
                                (unsigned long long)le64(p));
            return p + 8;
        ...
        case MT_NEWDECIMAL: {
            ...
            int scale     = (meta >> 8) & 0xFF;

            char dec[128];
            int consumed = newdecimal_bin_to_str(p, precision, scale, dec, sizeof(dec));
            if (consumed <= 0) {
                /* fallback to null but DON'T break parsing */
                *offset += snprintf(json_buf + *offset, buf_size - *offset, "null");
                return p; /* caller will detect no-progress; better to advance safely */
            }

            /*
             * Output as JSON string to preserve exact precision (recommended for decimals).
             * If you prefer numeric JSON (no quotes), remove the quotes below.
             */
            *offset += snprintf(json_buf + *offset, buf_size - *offset, "\"%s\"", dec);
            return p + consumed;
        }
        ...
        case MT_TIMESTAMP2: {
            ...
            if (meta > 0) {
                ...
                uint32_t frac = 0;
                for (int i = 0; i < frac_bytes; i++) frac = (frac << 8) | *p++;
                ...
                if (tm) {
                    ...
                    *offset += snprintf(json_buf + *offset, buf_size - *offset, "\"%u\"", sec);
                }
            }
            return p;
        }
        ...
        case MT_DATETIME2: {
            uint64_t val = 0;
            for (int i = 0; i < 5; i++) val = (val << 8) | *p++;
            val -= 0x8000000000LL;

            int ymd = (int)(val >> 17);
            int ym  = ymd >> 5;
            int hms = (int)(val & 0x1FFFF);
        ...
        case MT_BLOB:
        case MT_JSON: {  /* <-- JSON handled here */
            /*
             * meta = number of bytes used to store length (1..4 typically)
             * value = [len(meta bytes LE)] [payload bytes]
             */
            uint32_t len = 0;
            ...
            /* Emit a hex preview so it never breaks parsing. */
            *offset += snprintf(json_buf + *offset, buf_size - *offset, "\"0x");

            /* Print up to N bytes to keep JSON small */
            const unsigned max_dump = 64; /* adjust if you want more */
            unsigned dump = (len > max_dump) ? max_dump : len;

            for (unsigned i = 0; i < dump && *offset < buf_size - 4; i++) {
                static const char hexdig[] = "0123456789abcdef";
                unsigned char b = p[i];
                if (*offset < buf_size - 3) {
                    json_buf[(*offset)++] = hexdig[(b >> 4) & 0xF];
                    json_buf[(*offset)++] = hexdig[b & 0xF];
                }
            }

            if (len > max_dump && *offset < buf_size - 6) {
                *offset += snprintf(json_buf + *offset, buf_size - *offset, "…");
            }
        ...
        case MT_ENUM: {
            ...
            if (pack_len == 1) { enum_val = *p; p += 1; }
            else { enum_val = le16(p); p += 2; }
            ...
            if (g_enum_cache && col_idx < g_map.ncols) {
                ...
                if (!cache->loaded && g_map.column_names && g_map.column_names[col_idx]) {
                    load_enum_values_for_column(g_map.db, g_map.tbl, g_map.column_names[col_idx], cache);
                }
                if (cache->loaded && enum_val >= 1 && enum_val <= (uint16_t)cache->count) {
        ...
        default:
            /* CRITICAL: never “return p” for non-null values — that breaks parsing */
            *offset += snprintf(json_buf + *offset, buf_size - *offset, "null");
            /* best-effort: treat unknown as zero-length to avoid infinite loop */
            return p;
    }
}
...
static const unsigned char* skip_column_value(const unsigned char *p, uint32_t col_idx)
{
    ...
    switch (real_type) {
        ...
        case MT_NEWDECIMAL: {
            int precision = meta & 0xFF;
            int scale = (meta >> 8) & 0xFF;
            int int_part = precision - scale;

            int int_groups = int_part / 9;
            int int_remainder = int_part % 9;
            int frac_groups = scale / 9;
            int frac_remainder = scale % 9;

            static const int remainder_bytes[] = {0,1,1,2,2,3,3,4,4,4};
            int total_bytes = int_groups * 4 + remainder_bytes[int_remainder] +
                              frac_groups * 4 + remainder_bytes[frac_remainder];
        ...
        default:
            /* Unknown type: do not advance (caller will detect no-progress if needed) */
            return p;
    }
}
                                      ...
                                      char *json_buf, size_t buf_size, size_t *json_offset)
{
    ...
    *json_offset += snprintf(json_buf + *json_offset, buf_size - *json_offset, "{");
    ...
    for (uint32_t i = 0; i < ncols; ++i) {
        ...
        size_t consumed = (size_t)(p - start_p);
...
static void publish_event(const char *db, const char *table, const char *event_json, const char *txn); // fwd
...
static void parse_query(const unsigned char *p, uint32_t len){
    ...
    if(status_vars_len > 0) {
        ...
        if(offset + status_vars_len <= len) p += status_vars_len;
        else return;
    }
    ...
    while(query_len > 0 && query[query_len - 1] == 0) query_len--;
    if(query_len == 0) return;

    const char *type = "QUERY";
    int is_begin = 0, is_commit = 0, is_rollback = 0, is_ddl = 0;

    if(strncasecmp(query, "BEGIN", 5) == 0) { type = "BEGIN"; is_begin = 1; }
    else if(strncasecmp(query, "COMMIT", 6) == 0) { type = "COMMIT"; is_commit = 1; }
    else if(strncasecmp(query, "ROLLBACK", 8) == 0) { type = "ROLLBACK"; is_rollback = 1; }
    else if(strncasecmp(query, "CREATE", 6) == 0) { type = "CREATE"; is_ddl = 1; }
    else if(strncasecmp(query, "ALTER", 5) == 0) { type = "ALTER"; is_ddl = 1; }
    else if(strncasecmp(query, "DROP", 4) == 0) { type = "DROP"; is_ddl = 1; }
    else if(strncasecmp(query, "TRUNCATE", 8) == 0) { type = "TRUNCATE"; is_ddl = 1; }
    else if(strncasecmp(query, "RENAME", 6) == 0) { type = "RENAME"; is_ddl = 1; }
    ...
    if(is_ddl && db_len > 0 && should_capture_ddl(db)) {
        ...
        for(int i = 0; i < (int)query_len && i < 1023 && j < 2046; i++) {
            char c = query[i];
            if(c == '"') { escaped_query[j++] = '\\'; escaped_query[j++] = '"'; }
            else if(c == '\\') { escaped_query[j++] = '\\'; escaped_query[j++] = '\\'; }
            else if(c == '\n') { escaped_query[j++] = '\\'; escaped_query[j++] = 'n'; }
            else if(c == '\r') { escaped_query[j++] = '\\'; escaped_query[j++] = 'r'; }
            else if(c == '\t') { escaped_query[j++] = '\\'; escaped_query[j++] = 't'; }
            else escaped_query[j++] = c;
        }
        escaped_query[j] = '\0';
    ...
    if(is_commit || is_rollback) {
        log_info("[txn:%s] Transaction %s", current_txn_id, is_commit ? "COMMITTED" : "ROLLED BACK");
        in_transaction = 0;
        current_txn_id[0] = '\0';
    }
}
...
static void publish_event(const char *db, const char *table,
    ...
    while (inst) {
        if (publisher_should_publish(inst, db)) {
            if (publisher_instance_enqueue(inst, &event) == 0) {
                log_trace("Dispatching event publisher=%s txn=%s db=%s table=%s binlog_file=%s position=%llu : %s",
                          inst->name, txn, db, table, current_binlog,
                          (unsigned long long)current_position, event_json);
                dispatched++;
            }
        } else {
                                        ...
                                        const char *db, const char *tbl)
{
    ...
    *json_offset += snprintf(json_buf + *json_offset, buf_size - *json_offset, ",\"primary_key\":[");
                            ...
                            uint32_t ncols, const unsigned char *present)
{
    ...
    append_primary_key_metadata(json_event, sizeof(json_event), &json_offset, g_map.db, g_map.tbl);

    json_offset += snprintf(json_event + json_offset, sizeof(json_event) - json_offset, ",\"rows\":[");
                             ...
                             const unsigned char *after_present)
{
    ...
    while(len >= conservative_min && json_offset < sizeof(json_event) - 4000){
        ...
        json_offset += snprintf(json_event + json_offset, sizeof(json_event) - json_offset, "{\"before\":");
        ...
        json_offset += snprintf(json_event + json_offset, sizeof(json_event) - json_offset, ",\"after\":");
...
static void parse_rows_event(uint8_t event_type, const unsigned char *payload, uint32_t payload_len)
{
    ...
    if(event_type == EVT_UPDATE_ROWSv1 || event_type == EVT_UPDATE_ROWSv2 ||
       event_type == EVT_MARIA_UPDATE_ROWS_COMPRESSED){
        ...
        before_present = p; p += bmp_len;
        after_present = p;  p += bmp_len;
    } else {
    ...
    if(event_type == EVT_MARIA_WRITE_ROWS_COMPRESSED ||
       ...
       event_type == EVT_MARIA_DELETE_ROWS_COMPRESSED){
        if(mariadb_decompress_rows(p, payload_len - (uint32_t)(p - payload), &dec, &row_len) != 0){
    ...
    if(event_type == EVT_WRITE_ROWSv1 || event_type == EVT_WRITE_ROWSv2 ||
       event_type == EVT_MARIA_WRITE_ROWS_COMPRESSED){
        parse_write_rows(row_data, row_len, ncols, before_present);
              ...
              event_type == EVT_MARIA_UPDATE_ROWS_COMPRESSED){
        parse_update_rows(row_data, row_len, ncols, before_present, after_present);
    } else {
...
static void parse_rotate(const unsigned char *p, uint32_t len){
    ...
    size_t name_len = 0;
    while(name_len < len && name_len < sizeof(current_binlog) - 1 && p[name_len] != 0) {
    ...
    switch(type){
        ...
        case EVT_XID: {
            ...
            if(in_transaction && current_txn_id[0]) {
                log_debug("[txn:%s] XID COMMIT (server_xid=%llu)", current_txn_id, (unsigned long long)xid);
...
static int stream_binlog(MYSQL *m, MYSQL_RPL *rpl){
    log_info("Streaming from %s @ %llu", rpl->file_name, (unsigned long long)rpl->start_position);
...
int main(int argc, char **argv){
    ...
    if(restore_position(start_file, &start_pos) != 0) {
        if(g_config.binlog_file[0]) {
            ...
            start_pos = (uint64_t)g_config.binlog_position;
        } else {
    ...
    rpl.file_name_length = (uint32_t)strlen(start_file);
    rpl.file_name = start_file;
    rpl.start_position = start_pos;
    rpl.server_id = (uint32_t)g_config.server_id;
    rpl.flags = 0;
    ...
    if (g_config.publisher_manager) {
        publisher_instance_t *inst = g_config.publisher_manager->instances;
        while (inst) {
            if (inst->started) publisher_instance_stop(inst);
            inst = inst->next;
        }
